...
#define PICTURE_H 227
#define PICTURE_W 227
int main(int argc, char** argv)
{
	...
	Mat logo=imread("logo.jpg");
       // resize(logo,logo,Size(413,227));
	namedWindow("LOGO",CV_WINDOW_NORMAL);
	namedWindow("DIAGNOSTIC SYSTEM",CV_WINDOW_NORMAL);
	imshow("LOGO",logo);
	char PipeCNN_result[100];
	char PipeCNN_time[100];
	char PipeCNN_topic[100];
	sprintf(PipeCNN_topic,"Diagnostic Results:");
	sprintf(PipeCNN_result,"Initial...");
	sprintf(PipeCNN_time,"Initial...");
	string filename="./data/data_alex/image.dat";
	VideoCapture cap(-1);
	if(!cap.isOpened())
	{
	...
	{
		frame1=frame.clone();
		putText(frame1,PipeCNN_topic,Point(20,40),FONT_HERSHEY_DUPLEX,1,Scalar(0,0,255));

		putText(frame1,PipeCNN_result,Point(20,80),FONT_HERSHEY_DUPLEX,1,Scalar(0,0,255));
		putText(frame1,PipeCNN_time,Point(20,120),FONT_HERSHEY_DUPLEX,1,Scalar(0,0,255));
		if(frame.empty())
		{
			printf("No frame captured\n");
			break;
		}
		imshow("DIAGNOSTIC SYSTEM",frame1);
		int key=waitKey(10);
		if((char)key==27)
			break;
		if((char)key==32)
		{
			resize(frame,photo,Size(PICTURE_H,PICTURE_W));
			imshow("PHOTO",photo);
			ofstream outfile;
			outfile.open(filename.c_str(),ios::binary);
			
			Mat aChannels[3];
			split(photo,aChannels);
			
			Mat_<int> achnl0=aChannels[0];
			achnl0-=128;
			Mat_<char> achnl00=achnl0;
			for(int r=0;r<achnl00.rows;r++)
				outfile.write(reinterpret_cast<const char*>(achnl00.ptr(r)),achnl00.cols);
			
			Mat_<int> achnl1=aChannels[1];
			achnl1-=128;
			Mat_<char> achnl11=achnl1;
			for(int r=0;r<achnl11.rows;r++)
				outfile.write(reinterpret_cast<const char*>(achnl11.ptr(r)),achnl11.cols);
			
			Mat_<int> achnl2=aChannels[2];
			achnl2-=128;
			Mat_<char> achnl22=achnl2;
			for(int r=0;r<achnl22.rows;r++)
				outfile.write(reinterpret_cast<const char*>(achnl22.ptr(r)),achnl22.cols);
	...
	// Kernel excutions main loops
			for(unsigned i = 0; i < num_devices; ++i) {
				...
				int abnormal,normal;
				abnormal=output_one_item[0];
				normal=output_one_item[1];
				sprintf(PipeCNN_result,"ABNORMAL = %d%, NORMAL = %d%",abnormal,normal);
			    sprintf(PipeCNN_time,"Inference time is %fs", time);
				//}
...
void verifyResult(int num)
{
	...
	softmax(output_reorder, output_one_item);
	int p1;
	p1=getProb(output_one_item);
	output_one_item[p1]=0;
